...
async function authGuardAndGetUserData() {
    ...
    try {
        ...
        console.log("Auth Guard: Usuário autenticado:", userData.name, "Role:", userData.role);
        return userData; 
...
async function renderMachinesView(container, user) {
    ...
    if (!tabsContainer || !screen || !btnCriarMaquina || !newTabBtn) {
        ...
        if (screen) {
            screen.innerHTML = "<p style='color:red;'>Erro ao carregar o componente.</p>";
        }
        return;
    }
...
function renderMachineList(screenElement, sector, user) {
    ...
    if (!sector.machines || sector.machines.length === 0) {
        ...
        machineHTML += '<div class="machine-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1rem;">';
...
async function renderCreateMachineForm(viewContainer, user) {
    ...
    if (!form || !selectSector || !btnCancelar) {
        console.error("Erro: Elementos do formulário de criação de máquina não encontrados.");
        return;
    }
    ...
    if (allSectorsCache.length > 0) {
        ...
        selectSector.innerHTML = '<option value="">Nenhum setor encontrado</option>';
    }
                    ...
                    <div class="info-item">
                        ...
                        <div style="display: flex; gap: 1rem; align-items: center;">
                            <button class="promote-btn logout-btn" 
                                ...
                                ${user.role === 'admin' ? 'Já é Admin' : 'Promover a Admin'}
                            </button>
